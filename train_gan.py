...
class TrainGAN():
    def __init__(self, opt, attributes, unseenAtt, unseenLabels, writter, seen_feats_mean, gen_type='FG'):
        ...
        self.netG = model.Generator(self.opt)
        ...
        if no_grad is True:
            with torch.no_grad():
                for i in range(nclass):
                    ...
                    output = self.netG(Variable(syn_noise), Variable(syn_att), syn_emb)
    ...
    def trainEpoch(self):
        ...
        for i in range(0, self.ntrain, self.opt.batch_size):
            ...
            for iter_d in range(self.opt.critic_iter):
                ...
                fake = self.netG(z_conc, label_conc, embedding_gcn_conc)
            ...
            self.tensorwrite.add_scalar("train_loss", errG.item(), self.epoch * self.ntrain + i)
