...
class TrainGAN():
    def __init__(self, opt, attributes, unseenAtt, unseenLabels, seen_feats_mean, gen_type='FG'):
        ...
        self.unseen_classifier = ClsUnseen(self.opt, unseenAtt)
        ...
        self.classifier = ClsModel(opt, num_classes=opt.nclass_all)
    ...
    def get_long_z(self):
        z = torch.cuda.FloatTensor(self.opt.batch_size, self.opt.resSize)
        # z.normal_(0, 1)
        z.uniform_(-1, 1)
        return z
    ...
    def latent_augmented_sampling(self):
        query = self.get_z_random_v2(self.opt.batch_size, self.opt.nz, 'uniform')
        ...
        for k in range(self.opt.num_negative):
            neg = self.get_z_random_v2(self.opt.batch_size, self.opt.nz, 'uniform')
            while (neg - query).abs().min() < self.opt.radius:
                neg = self.get_z_random_v2(self.opt.batch_size, self.opt.nz, 'uniform')
            negs.append(neg)
        return query, pos, negs

    def get_z_random_v2(self, batchSize, nz, random_type='uniform'):
        z = torch.cuda.FloatTensor(batchSize, nz)
        #z.copy_(torch.randn(batchSize, nz))
        z.copy_(torch.rand(batchSize, nz)*2-torch.ones(batchSize, nz))
        return z
    ...
    def trainEpoch(self):
        ...
        for i in range(0, self.ntrain, self.opt.batch_size):
            ...
            for iter_d in range(self.opt.critic_iter):
                ...
                # z_random31 = self.get_long_z()
                # z_random41 = self.get_z_random()
                # criticG_fake31 = self.netD(z_random31, z_random41)
                # criticG_fake31 = criticG_fake31.log().mean()
                # uniform_reg1 = criticG_fake31
                # uniform_reg1.backward()
            ...
            criticG_fake = criticG_fake.log().mean()
            G_cost = criticG_fake

            ##todo
            criticG_fake2 = self.netD(fake2[:input_resv.size(0)], input_attv)
            #print(fake2[:input_resv.size(0)].shape)
            #print(input_attv.shape)
            ...
            ##
            z_random3 = self.get_long_z()
            z_random4 = self.get_z_random()
            criticG_fake3 = self.netD(z_random3, z_random4)
            criticG_fake3 = criticG_fake3.mean()
            uniform_reg = criticG_fake3
            ...
            errG = - G_cost - G_cost2 - uniform_reg + c_errG + loss_lz + fake_inter_contras_loss
