...
# from src.models.TransDecoder_local import TransDecoder
...
#         self.encoder = Encoder(ch = net_args['ch'], out_ch = net_args['out_ch'], num_res_blocks = net_args['num_res_blocks'], 
#                     ch_mult= net_args['ch_mult'], in_channels = net_args['in_channels'], resolution = net_args['resolution'], 
#                     z_channels = net_args['z_channels'], double_z = net_args['double_z'])
        
        self.encoder = TransEncoder(img_size = net_args['resolution'], in_chans = net_args['in_channels'], 
                                    out_chans = net_args['z_channels'], embed_dim = 192, depth = 6, num_heads = 4)
        
#         self.decoder = TransDecoder(img_size = net_args['resolution'], in_chans = net_args['z_channels'], 
...
#                                                net_args['nhead'], net_args['dim_feedforward'])
        self.style_mixer = StyleTransDecoder(img_size = net_args['resolution'], in_chans = net_args['z_channels'], 
                           in_chans_style = net_args['d_model'], out_chans=net_args['in_channels'], embed_dim=192,
                           n_emb_style = net_args['n_emb_style'], depth=6, num_heads=4, use_pos_embed = net_args['use_pos_embed'])
...
#             content = content.reshape(B, C, _H, _W).contiguous()
